...
##### Display graphs
st.write("**Visualization1**")
st.write(vis1)

st.write("**Visualization2**")
st.write(vis2)

st.write("**Visualization3**")
st.write(vis3)
...
chart = alt.Chart(df3).transform_filter(
    alt.datum.emoji == emoji_select
).mark_line().encode(
    alt.X('datetime:T', axis=alt.Axis(tickCount=5)),
    alt.Y('tweet_count:Q')
)

circles = alt.Chart(df3).transform_filter(
    alt.datum.emoji == emoji_select
).mark_circle(color="black",opacity=0.7).encode(
    alt.X('datetime:T'),
    alt.Y('tweet_count:Q'),
    opacity = brushCondition
).add_selection(brush)
...
st.write("**Visualization3 (Streamlit Version)**")
st.write(vis4)